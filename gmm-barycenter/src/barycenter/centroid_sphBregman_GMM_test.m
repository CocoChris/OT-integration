function [c, X] = centroid_sphBregman_GMM_test(stride, instanceW, supp, w, c0, options) 
...
%   disp('c.supp(1:d,:)'),disp(c.supp(1:d,:));
%   disp('supp(1:d,:)'),disp(supp(1:d,:));
  C = pdist2(c.supp(1:d,:)', supp(1:d,:)', 'euclidean').^2;
%   disp('c.supp((d+1):end,:)'),disp(c.supp((d+1):end,:));
%   disp('supp((d+1):end,:)'),disp(supp((d+1):end,:));
  C = C + gaussian_wd_test(c.supp((d+1):end,:), supp((d+1):end,:));
%   disp('C'),disp(C);
...
%         disp("X"),disp(X);
%         disp("suppW"),disp(suppW);
        tmpX = bsxfun(@times, X, suppW);
%         disp("tmpX"),disp(tmpX);
        c.supp(1:d,:) = supp(1:d,:) * tmpX' ./ repmat(sum(tmpX,2)', [d, 1]);
%         disp("done")
        c.supp((d+1):end,:) = gaussian_mean_test(supp((d+1):end,:), tmpX, c.supp((d+1):end,:));
%         disp("c.supp")
%         disp(c.supp)
%         disp("supp")
%         disp(supp)
