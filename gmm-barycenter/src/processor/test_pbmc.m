...
prefix='/Users/coco/Workspace/R/barycenter/data/pbmc/';
...
for i=1:dataset_num
    cluster_label_filename=strcat(prefix, 'clusters_', tech{i}, '.txt');
    cca_filename=strcat(prefix, 'cca/', tech{i}, '.txt');
...
cluster_distribution1 = zeros(cluster_num, cluster_num);
...
cluster_distribution2 = zeros(cluster_num, cluster_num);
...
figure;
subplot(2,2,1)
gscatter(mappedX(1:index_list(1),1), mappedX(1:index_list(1),2), data_before_ot(1:index_list(1),1));title(strcat(tech{1}, ' data'));
subplot(2,2,2)
gscatter(mappedX((index_list(1)+1):index_list(2),1), mappedX((index_list(1)+1):index_list(2),2), data_before_ot((index_list(1)+1):index_list(2),1));title(strcat(tech{2}, ' data'));
subplot(2,2,3)
gscatter(mappedX(1:index_list(2),1), mappedX(1:index_list(2),2), dataset_label);title(strcat('colored by dataset'));
subplot(2,2,4)
gscatter(mappedX((index_list(2)+1):end,1), mappedX((index_list(2)+1):end,2), data_after_ot(:,1));title(strcat('colored by cluster'));
